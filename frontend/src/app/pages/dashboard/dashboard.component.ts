    ...
    <div class="dashboard-container fade-in">
      ...
      <div class="performance-grid">
        ...
        <div class="perf-card glass-card">
          <div class="perf-info">
            <h3>Memory</h3>
            <div class="value">2.4 GB / 4 GB</div>
          </div>
          <div class="perf-chart">
          ...
          <div class="instances-table">
            ...
            <div class="table-row" *ngFor="let app of apps">
              ...
              <div class="status-cell">
                <span class="pill" [class.running]="app.status === 'running'">
                  {{ app.status === 'running' ? 'Actif' : 'Arrêté' }}
                </span>
              </div>
        ...
        <div class="side-panel">
          ...
          <div class="terminal-panel">
            ...
            <div class="terminal-body">
              <div class="line"><span class="prompt">root@nexgen-cloud:~$</span> docker ps</div>
              ...
              <div class="line"><span class="prompt">root@nexgen-cloud:~$</span> <span class="cursor">_</span></div>
            </div>
          </div>
        </div>
      </div>
...
export class DashboardComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    // Simulate live charts
    setInterval(() => {
      this.cpuHistory.shift();
      this.cpuHistory.push(Math.floor(Math.random() * 20) + 30);
      this.ramHistory.shift();
      this.ramHistory.push(Math.floor(Math.random() * 10) + 55);
      this.netHistory.shift();
      this.netHistory.push(Math.floor(Math.random() * 40) + 20);
    }, 2000);
  }

  fetchApps() {
    this.http.get<any[]>('http://localhost:3000/api/apps', { headers: this.getHeaders() })
      .subscribe({
        next: (data) => {
  ...
  stopApp(id: number) {
    this.http.post(`http://localhost:3000/api/apps/${id}/stop`, {}, { headers: this.getHeaders() })
      .subscribe({
        next: () => this.fetchApps(),
        error: (err) => alert("Erreur lors de l'arrêt")
      });
  }

  deleteApp(id: number) {
    if(confirm('Êtes-vous sûr de vouloir supprimer cette application ?')) {
      this.http.delete(`http://localhost:3000/api/apps/${id}`, { headers: this.getHeaders() })
        .subscribe({
          next: () => this.fetchApps(),
